...
import { IGNORED_FOLDERS, IGNORED_FILES, BLOCKED_EXTENSIONS } from './file.config.js';
...
const CloseIcon = ({ className = "w-3 h-3" }) => (<svg className={className} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M18 6L6 18M6 6L18 18" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" /></svg>);
const InfoIcon = ({ className = "w-4 h-4" }) => (<svg className={className} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" /></svg>);
const ACCEPTED_FILE_TYPES = ".py,.js,.ts,.tsx,.json,.html,.css,.md,.csv,.txt,.pdf,image/png,image/jpeg,image/gif,image/webp";
...
const AttachmentChip = ({ file, onRemove }: { file: File, onRemove?: () => void }) => {
    ...
    return (
        <div className="flex items-center gap-1 text-sm max-w-xs pl-2 pr-1 py-1 rounded-full bg-gray-200 text-gray-700 dark:bg-slate-700 dark:text-slate-300">
            <Icon />
            <span className="truncate" title={displayName}>{displayName}</span>
            {onRemove && (
                <button 
                    onClick={onRemove} 
                    className="p-0.5 rounded-full flex items-center justify-center text-red-500 hover:bg-red-500/20 transition-colors"
    ...
    switch (part.type) {
        case 'title': return (
            <div className="border-b-2 border-sky-500 dark:border-sky-400 pb-3 mb-4">
                <h1 className="text-4xl font-bold break-words title"><ReactMarkdown remarkPlugins={markdownPlugins} rehypePlugins={htmlPlugins}>{part.content || ''}</ReactMarkdown></h1>
                {part.subtitle && <div className="text-lg mt-1 subtitle"><ReactMarkdown remarkPlugins={markdownPlugins} rehypePlugins={htmlPlugins}>{part.subtitle}</ReactMarkdown></div>}
            </div>
        );
        ...
        case 'quote_heading': return (
            <blockquote className="my-4 border-l-4 p-4 rounded-r-lg quote-heading-container">
                <div className="text-lg font-medium italic quote-text"><ReactMarkdown remarkPlugins={markdownPlugins} rehypePlugins={htmlPlugins}>{part.content || ''}</ReactMarkdown></div>
        ...
        case 'code':
            ...
            return (
                <div className="relative group my-4 rounded-md bg-[#282c34] overflow-x-auto">
                    ...
                    <SyntaxHighlighter language={part.language === 'error' ? 'bash' : (part.language || 'text')} style={oneDark} showLineNumbers customStyle={{ margin: 0, padding: '1rem', paddingTop: '1rem' }}>
                        {codeContent}
                    </SyntaxHighlighter>
                </div>
            );
...
export default function App() {
    ...
    const handleDeleteChat = async (chatIdToDelete: string) => {
        ...
        try {
            ...
            if (errorMessage.includes("Authentication error")) {
                ...
                setError(`Failed to delete chat: ${errorMessage}. Chat list restored.`);
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Если есть отклонённые файлы, показываем ошибку
        if (rejectedFiles.length > 0) {
            const fileList = rejectedFiles.join(', ');
            // Сообщение на простом английском (A1-A2)
            setError(`Error: For security, you cannot upload these files: ${fileList}. Please upload text, document or image files.`);
        }

        // Добавляем только разрешённые файлы
    ...
    const handleFolderChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		// Add rules from user's .gitignore if it exists
        if (gitignoreFile) {
			try {
		...
		fileList.forEach(file => {
            ...
            // Pathspec needs the path relative to the root of the upload, without the top-level folder
...
// src/App.tsx
	...
	const handlePaste = (e: React.ClipboardEvent<HTMLTextAreaElement>) => {
		...
		// Instead of a specific list, we now check for any item of the 'file' kind.
		// This makes the function support all file types that the browser can handle.
		for (const item of e.clipboardData.items) {
			if (item.kind === "file") {
				const file = item.getAsFile();
				if (file) {
					// We still create a unique name to avoid conflicts and keep the UI clean.
		...
		if (filesToUpload.length > 0) {
			e.preventDefault(); // Stop the browser from pasting the file path as text.
    ...
    const handleSubmit = async () => {
		...
		try {
			...
			if (rememberContext && activeChatContent.conversation.length > 0) {
			...
			setActiveChatContent(prev => {
				if (!prev) return null;
				return { ...prev, conversation: [...prev.conversation, errorTurn] };
			});
		} finally {
			setIsLoading(false);
		}
            ...
            <div className={`h-screen flex flex-col transition-colors duration-300 ${isDarkMode ? "bg-gray-900 text-white" : "bg-gray-50 text-gray-900"}`}>
				...
				<main className="max-w-7xl mx-auto grid flex-1 grid-cols-1 lg:grid-cols-4 gap-6 p-6 min-h-0">
					<aside className="lg:col-span-1">
						<div className={`p-6 h-full rounded-xl shadow-sm border border-gray-700/30 dark:border-gray-700 ${isDarkMode ? "bg-gray-800/60" : "bg-white/60"} backdrop-blur-sm`}>
					...
					<div className={`lg:col-span-2 rounded-xl shadow-sm border border-gray-700/30 dark:border-gray-700 flex flex-col min-h-0 ${isDarkMode ? "bg-gray-800/60" : "bg-white/60"} backdrop-blur-sm`}>
									...
									{turn.type === 'user' ? (
										<div className="flex flex-col items-end gap-2 w-full">
											<div className="user-bubble">
												{/* 
												  ИЗМЕНЕНИЕ: ReactMarkdown заменен на div для буквального отображения текста.
												  Класс 'whitespace-pre-wrap' сохраняет переносы строк и пробелы.
												*/}
												{turn.prompt && (
												...
												{turn.attachments && turn.attachments.length > 0 && (
													<div className="mt-2 flex flex-wrap gap-2 border-t border-white/20 pt-2">
														{turn.attachments.map((file, idx) => (<AttachmentChip key={idx} file={{ name: file.name } as File} />))}
													</div>
												)}
											</div>
										</div>
						...
						<div className="p-4 bg-gray-100/50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700/50">
							<div className="input-area-container">
								<textarea
									value={inputText}
									onChange={(e) => setInputText(e.target.value)}
									onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSubmit(); } }}
									onPaste={handlePaste} // The updated paste handler is attached here.
									placeholder="Ask Gemini something..." // The new, simpler placeholder.
									rows={3}
								...
								<input
									...
									className="hidden"
									id="file-upload"
									accept={ACCEPTED_FILE_TYPES} // Using the constant for accepted types.
								/>
								{attachedFiles.length > 0 && (
									<div className="px-3 pt-3 flex flex-wrap gap-2 border-t border-gray-200 dark:border-gray-700/50">
										{attachedFiles.map((file, index) => {
											// Display a thumbnail for image files.
											if (file.type.startsWith("image/")) {
												const previewUrl = URL.createObjectURL(file);
												return (
													<div key={index} className="relative group w-24 h-24 rounded-lg overflow-hidden border-2 border-gray-300 dark:border-gray-600 shadow-sm">
														...
														>
															<button 
																...
																aria-label={`Remove file ${file.name}`} // English label.
															>
																<TrashIcon className="w-5 h-5" />
															</button>
														</div>
														<div className="absolute bottom-0 left-0 right-0 bg-black/50 text-white text-xs p-1 truncate" title={file.name}>
															{file.name}
														</div>
													</div>
												);
											}
											// Display a chip for other file types (PDF, code, etc.).
											return (
												<AttachmentChip 
													key={`${file.name}-${index}`} 
													file={file} 
								...
								<div className="flex items-center justify-between p-2">
									<div className="flex items-center gap-1">
										<button onClick={handleUploadFileClick} className="input-tool-button" aria-label="Attach file"><PaperclipIcon /></button>
										<button onClick={handleUploadFolderClick} className="input-tool-button" aria-label="Attach folder"><FolderIcon /></button>
										<button onClick={() => setIsRepoModalOpen(true)} className="input-tool-button" aria-label="Attach GitHub repository"><GithubIcon /></button>
										...
										<div className="flex items-center gap-2">
											<label htmlFor="remember-context-toggle" className="flex items-center cursor-pointer select-none">
												<span className="mr-3 text-sm font-medium text-gray-800 dark:text-gray-300">{config.dialog.historyToggleLabel}</span>
												<div className="relative">
													<input type="checkbox" id="remember-context-toggle" checked={rememberContext} onChange={() => setRememberContext(!rememberContext)} className="sr-only peer" />
													<div className="block w-11 h-6 bg-gray-300 rounded-full peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-offset-2 peer-focus:ring-offset-gray-100 dark:peer-focus:ring-offset-gray-900 peer-focus:ring-blue-500 dark:bg-gray-600 peer-checked:bg-blue-600 transition"></div>
													<div className="absolute block w-4 h-4 mt-1 ml-1 transform bg-white rounded-full left-0 top-0 transition-transform peer-checked:translate-x-full"></div>
												</div>
											</label>
											<div className="relative group flex items-center">
												<InfoIcon className="w-5 h-5 text-gray-400 dark:text-gray-500" />
												<div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-3 bg-slate-800 text-white text-xs text-center rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
									...
									<button
										...
										disabled={(!inputText.trim() && attachedFiles.length === 0) || isLoading || isContentLoading}
					...
					<aside className="lg:col-span-1 flex flex-col min-h-0">
						<div className={`p-4 h-full rounded-xl shadow-sm border border-gray-700/30 dark:border-gray-700 ${isDarkMode ? "bg-gray-800/60" : "bg-white/60"} backdrop-blur-sm flex flex-col`}>
							{isAuthLoading ? (
										...
										{chats.length > 0 ? (
											<div style={{ height: `${rowVirtualizer.getTotalSize()}px`, width: '100%', position: 'relative' }}>
												{rowVirtualizer.getVirtualItems().map((virtualItem) => {
													...
													if (!chat) return null;
													return (
														<div key={chat.id} style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: `${virtualItem.size}px`, transform: `translateY(${virtualItem.start}px)`, paddingBottom: '8px' }}>
															<div onClick={() => editingChatId !== chat.id && setActiveChatId(chat.id)} onDoubleClick={() => handleStartEditing(chat)} className={`group p-3 rounded-lg cursor-pointer transition-colors relative h-full flex flex-col justify-center ${activeChatId === chat.id ? "bg-blue-600/20" : "hover:bg-gray-700/30"}`}>
																{editingChatId === chat.id ? (
																	...
																	<>
																		...
																		<div className="absolute top-1/2 right-2 -translate-y-1/2 flex items-center opacity-0 group-hover:opacity-100 focus-within:opacity-100">
																			<button onClick={(e) => { e.stopPropagation(); handleStartEditing(chat); }} className="p-1 rounded-md hover:bg-gray-500/30 transition-opacity" aria-label="Rename chat"><EditIcon className="w-4 h-4 text-slate-400" /></button>
