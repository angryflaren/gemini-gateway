...
export default function App() {
    ...
    // --- ИЗМЕНЕНИЕ: Инициализируем состояние локальным чатом ---
    ...
    // --- ИЗМЕНЕНИЕ: Обновленная логика загрузки контента чата ---
    useEffect(() => {
        const loadChatContent = async () => {
            // Не загружаем ничего, если это локальный чат или нет пользователя/ID
            ...
            try {
                ...
                setActiveChatContent(createLocalChat()); // Возвращаемся к локальному чату в случае ошибки
    ...
    useEffect(() => {
        if (user && isInitialized) {
            ...
            // При выходе из системы или если пользователь не вошел, сбрасываем состояние к локальному чату
            setChats([]);
            setActiveChatId(LOCAL_CHAT_ID);
            setActiveChatContent(createLocalChat());
        }
