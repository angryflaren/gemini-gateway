...
// --- ИКОНКИ И ВСПОМОГАТЕЛЬНЫЕ КОМПОНЕНТЫ (без изменений) ---
...
const AttachmentChip = ({ file, onRemove }: { file: File, onRemove?: () => void }) => { const isRepo = file.name.startsWith('gh_repo:::'); const isFolder = file.name.endsWith('.zip'); let displayName: string = file.name; let Icon = FileIconForAttachment; if (isRepo) { displayName = file.name.replace('gh_repo:::', '').replace('_context.txt', '').replace(/---/g, '/'); Icon = GithubIconForAttachment; } else if (isFolder) { displayName = file.name.replace('.zip', ''); Icon = FolderIconForAttachment; } return (<div className="flex items-center gap-1 text-sm max-w-xs pl-2 pr-1 py-1 rounded-full bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-slate-300"> <Icon /> <span className="truncate" title={displayName}>{displayName}</span> {onRemove && <button onClick={onRemove} className="text-red-500 hover:text-red-400 font-bold text-lg leading-none flex items-center justify-center w-4 h-4">×</button>} </div>); };
const ResponseBlock = React.memo(({ part, isDarkMode }: { part: ResponsePart; isDarkMode: boolean }) => { const [copied, setCopied] = useState(false); const handleCopy = (contentToCopy: string) => { if (typeof contentToCopy !== 'string') return; navigator.clipboard.writeText(contentToCopy).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000); }); }; const markdownPlugins = [remarkMath, remarkGfm]; const htmlPlugins = [rehypeKatex]; switch (part.type) { case 'title': return (<div className="border-b-2 border-sky-500 dark:border-sky-400 pb-3 mb-4"><h1 className="text-4xl font-bold break-words title"><ReactMarkdown remarkPlugins={markdownPlugins} rehypePlugins={htmlPlugins}>{part.content}</ReactMarkdown></h1>{part.subtitle && <p className="text-lg mt-1 subtitle"><ReactMarkdown remarkPlugins={markdownPlugins} rehypePlugins={htmlPlugins}>{part.subtitle}</ReactMarkdown></p>}</div>); case 'heading': return <h2 className="text-2xl font-bold border-b dark:border-slate-700 pb-2 pt-4 break-words"><ReactMarkdown remarkPlugins={markdownPlugins} rehypePlugins={htmlPlugins}>{part.content}</ReactMarkdown></h2>; case 'subheading': return <h3 className="text-xl font-semibold pt-3 break-words"><ReactMarkdown remarkPlugins={markdownPlugins} rehypePlugins={htmlPlugins}>{part.content}</ReactMarkdown></h3>; case 'annotated_heading': return (<div className="flex items-center gap-3 pt-4"><h4 className="text-lg font-semibold break-words">{part.content}</h4><span className="info-tag">{part.tag}</span></div>); case 'quote_heading': return (<blockquote className="my-4 border-l-4 p-4 rounded-r-lg quote-heading-container"> <p className="text-lg font-medium italic quote-text"> <ReactMarkdown remarkPlugins={markdownPlugins} rehypePlugins={htmlPlugins}>{part.content}</ReactMarkdown> </p> {part.source && (<footer className="block text-right text-sm mt-2 not-italic quote-cite"> — <cite>{part.source}</cite> </footer>)} </blockquote>); case 'text': return (<ReactMarkdown className="leading-relaxed break-words prose dark:prose-invert max-w-none" remarkPlugins={markdownPlugins} rehypePlugins={htmlPlugins}>{part.content}</ReactMarkdown>); case 'code': const codeContent = String(part.content || ''); return (<div className="relative group my-4 rounded-md bg-gray-200 dark:bg-[#282c34] overflow-x-auto"> <button onClick={() => handleCopy(codeContent)} className="absolute top-2 right-2 p-1.5 rounded-md bg-black/40 text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity z-10 hover:bg-black/60" aria-label="Copy code">{copied ? <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg> : <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>}</button> <SyntaxHighlighter language={part.language === 'error' ? 'bash' : part.language} style={isDarkMode ? oneDark : oneLight} showLineNumbers customStyle={{ margin: 0, padding: '1rem', paddingTop: '1rem', backgroundColor: 'transparent' }}>{codeContent}</SyntaxHighlighter> </div>); case 'math': return <BlockMath math={part.content} />; case 'list': return (<ul className="list-disc pl-6 space-y-2 prose dark:prose-invert max-w-none"> {Array.isArray(part.items) && part.items.map((item, i) => (<li key={i}> <ReactMarkdown remarkPlugins={markdownPlugins} rehypePlugins={htmlPlugins}> {item} </ReactMarkdown> </li>))} </ul>); default: const unknownPart = part as any; return (<div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert"><strong className="font-bold">Unknown Block Type!</strong><span className="block sm:inline"> An unknown block type '{unknownPart?.type}' was received.</span><pre className="mt-2 text-xs">{JSON.stringify(unknownPart, null, 2)}</pre></div>); } });
const HelpModal = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) => { if (!isOpen) return null; return (<div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4" onClick={onClose}> <div className="bg-white dark:bg-slate-900/80 dark:backdrop-blur-sm dark:border dark:border-gray-700 rounded-lg shadow-xl p-6 w-full max-w-2xl" onClick={e => e.stopPropagation()}> <h3 className="text-lg font-bold mb-4 text-gray-900 dark:text-slate-100">{config.helpModal.title}</h3> <div className="prose prose-sm dark:prose-invert max-w-none space-y-4"> <p>{config.helpModal.introduction}</p> <div><h4 className="font-semibold">{config.helpModal.apiKeyTitle}</h4><p>{config.helpModal.apiKeySection}</p></div> <div><h4 className="font-semibold">{config.helpModal.filesTitle}</h4><p>{config.helpModal.filesSection}</p></div> <div><h4 className="font-semibold">{config.helpModal.repoTitle}</h4><ReactMarkdown>{config.helpModal.repoSection}</ReactMarkdown></div> <div><h4 className="font-semibold">{config.helpModal.contactTitle}</h4><ReactMarkdown>{config.helpModal.contactSection}</ReactMarkdown></div> </div> <div className="flex justify-end mt-6"><button onClick={onClose} className="px-4 py-2 text-sm rounded-md bg-blue-600 text-white hover:bg-blue-700">{config.helpModal.closeButton}</button></div> </div> </div>); };
const RepoCloneModal = ({ isOpen, onClose, onSubmit, isCloning }: { isOpen: boolean, onClose: () => void, onSubmit: (url: string) => void, isCloning: boolean }) => { const [url, setUrl] = useState(""); if (!isOpen) return null; return (<div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4" onClick={onClose}> <div className="bg-white dark:bg-slate-900/80 dark:backdrop-blur-sm dark:border dark:border-gray-700 rounded-lg shadow-xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}> <h3 className="text-lg font-bold mb-4 text-gray-900 dark:text-slate-100">{config.repoModal.title}</h3> <p className="text-sm text-gray-600 dark:text-slate-400 mb-4">{config.repoModal.description}</p> <input type="text" value={url} onChange={(e) => setUrl(e.target.value)} placeholder={config.repoModal.placeholder} className="w-full px-4 py-2 rounded-lg border focus:outline-none focus:ring-2 bg-gray-50 border-gray-300 focus:border-blue-500 focus:ring-blue-500/20 dark:bg-gray-700/50 dark:border-gray-600 dark:focus:border-blue-500 dark:text-white" /> <div className="flex justify-end gap-4 mt-6"> <button onClick={onClose} disabled={isCloning} className="px-4 py-2 text-sm rounded-md text-gray-700 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-gray-800">{config.repoModal.cancelButton}</button> <button onClick={() => onSubmit(url)} disabled={isCloning || !url} className="px-4 py-2 text-sm rounded-md bg-blue-600 text-white hover:bg-blue-700 disabled:bg-slate-500"> {isCloning ? config.repoModal.submitButtonCloning : config.repoModal.submitButton} </button> </div> </div> </div>); };
const AuthDisplay = ({ user, onLogin, onLogout, isLoading, isReady }: { user: UserProfile | null, onLogin: () => void, onLogout: () => void, isLoading: boolean, isReady: boolean }) => { if (user) { return (<div className="relative group"> <img src={user.imageUrl} alt={user.name} className="w-8 h-8 rounded-full cursor-pointer" /> <div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-gray-800 rounded-md shadow-lg py-1 hidden group-hover:block z-20"> <div className="px-4 py-2 text-sm text-gray-700 dark:text-gray-300 border-b dark:border-gray-600"> <p className="font-semibold truncate">{user.name}</p> <p className="text-xs truncate">{user.email}</p> </div> <a href="#" onClick={(e) => { e.preventDefault(); onLogout(); }} className="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"> Sign Out </a> </div> </div>); } return (<button onClick={onLogin} disabled={isLoading || !isReady} className="p-2 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-800/50 transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-wait" aria-label="Sign in with Google"> {isLoading ? <SpinnerIcon /> : <GoogleIcon />} </button>); }
...
export default function App() {
    ...
    // Загрузка списка чатов при логине
    useEffect(() => {
            ...
            if (user && isInitialized) {
                ...
                try {
                    ...
                    if (chatList.length > 0) {
                        ...
                        // Если чатов нет, создаем "пустое" состояние для начала работы
    ...
    // Загрузка контента выбранного чата
    useEffect(() => {
        const loadChatContent = async () => {
            if (activeChatId) {
                setIsContentLoading(true);
                try {
                    const content = await getChatContent(activeChatId);
    ...
    const handleCreateNewChat = useCallback(async () => {
        ...
        try {
            ...
            setActiveChatId(newChat.id); // Это вызовет useEffect для загрузки контента
        } catch (err) {
    ...
    const handleStartEditing = (chat: Chat) => { setEditingChatId(chat.id); setEditingChatName(chat.name); };
    const handleCancelEditing = () => { setEditingChatId(null); setEditingChatName(""); };

    const handleRenameChat = async (e?: React.FormEvent) => {
        if (e) e.preventDefault();
        const originalId = editingChatId;
        const newName = editingChatName.trim();
        const originalChat = chats.find(c => c.id === originalId);

        if (!originalId || !newName || newName === originalChat?.name) {
            handleCancelEditing();
            return;
        }
        ...
        if (activeChatContent && activeChatContent.id === originalId) {
            setActiveChatContent(prev => ({ ...prev!, name: newName }));
        }
        handleCancelEditing();
        ...
        try {
            ...
            if (activeChatContent && activeChatContent.id === originalId) {
                setActiveChatContent(prev => ({ ...prev!, name: originalChat!.name }));
            }
        }
    };
    ...
    const handleCloneRepo = async (url: string) => {
        ...
        try {
            ...
            if (!response.ok) { const errorData = await response.json(); throw new Error(errorData.detail || 'Failed to clone repository'); }
    ...
    const handleSubmit = async () => {
        ...
        let chatContext = activeChatContent;
        let isNewChat = false;
        
        // 1. Создание нового чата, если необходимо (стабильный, атомарный шаг)
        if (user && !activeChatId) {
            isNewChat = true;
            try {
                const chatTitle = inputText.substring(0, 40) || `Chat @ ${new Date().toLocaleTimeString()}`;
                const newChat = await createNewChatFile(chatTitle);
                chatContext = { id: newChat.id, name: newChat.name, conversation: [] };
                
                // Обновляем UI одним пакетом
                setChats(prev => [newChat, ...prev]);
                setActiveChatId(newChat.id);

            } catch (err) {
                console.error("Failed to create initial chat file:", err);
                setError("Could not create a new chat on Google Drive.");
                setIsLoading(false);
                return;
            }
        }

        // 2. Оптимистичное обновление UI (без "прыжков")
        const userTurn: ConversationTurn = { type: 'user', prompt: inputText, attachments: [], timestamp: new Date().toLocaleTimeString() };
        const updatedConversation = [...(chatContext?.conversation || []), userTurn];
        
        // Временно обновляем контент для отображения
        const tempChatContent: ChatContent = {
            id: chatContext?.id || '',
            name: chatContext?.name || 'New Chat',
            conversation: updatedConversation
        };
        setActiveChatContent(tempChatContent);
        ...
        try {
            ...
            const aiParts: ResponsePart[] = await response.json();
            const aiTurn: ConversationTurn = { type: 'ai', parts: aiParts, timestamp: new Date().toLocaleTimeString() };

            // 4. Финальное обновление состояния и сохранение
            const finalConversation = [...updatedConversation, aiTurn];
            let finalName = tempChatContent.name;
            if (isNewChat) {
                finalName = inputText.substring(0, 40) || `Chat @ ${new Date().toLocaleTimeString()}`;
            }

            const finalChatContent: ChatContent = { ...tempChatContent, name: finalName, conversation: finalConversation };
            setActiveChatContent(finalChatContent);
            
            // Обновляем имя в списке чатов, если оно изменилось
            if (finalName !== tempChatContent.name) {
                setChats(prev => prev.map(c => c.id === finalChatContent.id ? { ...c, name: finalName } : c));
            }
            
            // Сохраняем на диск, если пользователь авторизован
            if (user && finalChatContent.id) {
                await saveChat(finalChatContent);
                if (isNewChat) {
                    await renameChatFile(finalChatContent.id, finalName);
                }
            }

        } catch (error) {
            const message = error instanceof Error ? error.message : "An unknown error occurred.";
            const errorTurn: ConversationTurn = { type: 'ai', parts: [{ type: 'code', language: 'error', content: `Request failed: ${message}` }], timestamp: new Date().toLocaleTimeString() };
            
            // Показываем ошибку, не удаляя запрос пользователя
                ...
                <main className="max-w-7xl mx-auto grid flex-1 grid-cols-1 lg:grid-cols-4 gap-6 p-6 min-h-0">
                    ...
                    <div className={`lg:col-span-2 rounded-xl shadow-sm border border-gray-700/30 dark:border-gray-700 flex flex-col min-h-0 ${isDarkMode ? "bg-gray-800/60" : "bg-white/60"}`}>
                        <div ref={chatContainerRef} className="flex-grow overflow-y-auto p-6 space-y-6 scrollbar-thin scrollbar-thumb-rounded scrollbar-thumb-gray-700 scrollbar-track-transparent">
                            {(isLoading && !activeChatContent?.conversation.length) || isContentLoading ? (
                                <div className="flex items-center justify-center h-full">
                                    <SpinnerIcon className="w-8 h-8 text-slate-400" />
                                </div>
                            ) : (activeChatContent?.conversation || []).length === 0 ? (
                                <div className="flex flex-col items-center justify-center h-full text-center opacity-70">
                                    <GemIcon className="w-16 h-16 mb-4" />
                                    <h3 className="text-lg font-medium mb-1">Start your conversation</h3>
                                    <p className="text-sm max-w-md">{user ? "Select a chat or start typing below to create a new one." : "Enter your prompt below to begin. Sign in to save your history."}</p>
                                </div>
                            ) : (
                            ...
                            {isLoading && activeChatContent?.conversation.length && (
                        ...
                        <div className="border-t border-gray-700/30 dark:border-gray-700 p-4 bg-gray-100/50 dark:bg-gray-900/50">
                            {attachedFiles.length > 0 && ( <div className="mb-3 flex flex-wrap gap-2"> {attachedFiles.map((file, index) => ( <AttachmentChip key={`${file.name}-${index}`} file={file} onRemove={() => removeFile(index)} /> ))} </div> )}
                    ...
                    <aside className="lg:col-span-1 flex flex-col gap-4">
                        <div className={`p-4 rounded-xl shadow-sm border border-gray-700/30 dark:border-gray-700 ${isDarkMode ? "bg-gray-800/60" : "bg-white/60"} flex-1 flex flex-col`}>
                            {isAuthLoading ? ( <div className="flex-1 flex items-center justify-center"><SpinnerIcon className="w-8 h-8" /></div> ) : user ? (
                                <>
                                    ...
                                    <div className="flex-1 overflow-y-auto space-y-2 pr-2 scrollbar-thin scrollbar-thumb-rounded scrollbar-thumb-gray-700 scrollbar-track-transparent">
                                        {chats.map((chat) => (
                                            <div key={chat.id} onClick={() => !editingChatId && setActiveChatId(chat.id)} onDoubleClick={() => handleStartEditing(chat)} className={`group p-3 rounded-lg cursor-pointer transition-colors relative ${activeChatId === chat.id ? "bg-blue-600/20" : "hover:bg-gray-700/30"}`}>
                                                {editingChatId === chat.id ? ( <form onSubmit={handleRenameChat}> <input ref={renameInputRef} type="text" value={editingChatName} onChange={(e) => setEditingChatName(e.target.value)} onBlur={() => handleRenameChat()} onKeyDown={(e) => { if (e.key === 'Escape') handleCancelEditing(); }} className="rename-input" /> </form>
                                                ) : (
                                                    <>
                                                        ...
                                                        <button onClick={(e) => { e.stopPropagation(); handleStartEditing(chat); }} className="absolute top-1/2 right-2 -translate-y-1/2 p-1 rounded-md opacity-0 group-hover:opacity-100 focus:opacity-100 hover:bg-gray-500/30 transition-opacity" aria-label="Rename chat">
                                ...
                                <div className="flex-1 flex flex-col items-center justify-center text-center opacity-70">
                                    ...
                                    <p className="text-sm max-w-md"> Sign in with your Google Account to automatically save and sync your chat history to Google Drive. </p>
                                    <button onClick={signIn} className="mt-4 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors flex items-center gap-2"> Sign in with Google </button>
                                </div>
                            )}
                        </div>
                    </aside>
