...
export default function App() {
    ...
    // Состояния для чатов и аутентификации
    ...
    useEffect(() => {
        initGoogleClient(
            ...
            () => {
                console.error("GAPI initialization failed.");
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Сохраняем в Drive если пользователь вошел
            if (user && isGapiReady) {
                const savedChatId = await saveChat(chatToSave);
                setActiveChat({ ...chatToSave, id: savedChatId });
                // Обновляем список чатов, чтобы показать новый или обновленный
                await refreshChats();
            } else {
